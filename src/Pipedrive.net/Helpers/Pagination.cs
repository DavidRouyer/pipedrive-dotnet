...
namespace Pipedrive
{
    internal static class Pagination
    {
        internal static IDictionary<string, string> Setup(IDictionary<string, string> parameters, ApiOptions options)
        {
            ...
            {
                parameters.Add("limit", options.PageSize.Value.ToString(CultureInfo.InvariantCulture));
            }

            if (options.StartPage.HasValue)
            {
                parameters.Add("start", options.StartPage.Value.ToString(CultureInfo.InvariantCulture));
            }
